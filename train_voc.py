...
train_dataset = VOCDataset(root_dir='C:/Users/86153/Desktop/hw2_v2/VOCdevkit/VOC2007',resize_size=[800,1333],
                           split='trainval',use_difficult=False,is_train=True,augment=transform)

model = FCOSDetector(mode="training").cuda() # 开启gpu
model = torch.nn.DataParallel(model) # 开启多卡运行
if pretrain:
    model.load_state_dict(torch.load('./checkpoint/model_{}.pth'.format(pretrain)))
...
if __name__ == '__main__':
    for epoch in range(EPOCHS):
        ...
        if pretrain:
            torch.save(model.state_dict(),"./checkpoint/model_{}.pth".format(epoch + pretrain + 1))
        else:
